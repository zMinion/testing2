...
class nggManageGallery {
	...
	{
		//$pages = get_pages(); //modified by zMinion
	...
	function post_processor_images() {
        ...
        if (isset ($_POST['addnewpage']))
        {
			...
			$page['post_excerpt']    = stripslashes($_POST['gallerydesc']); //added by zMinion

            $page['post_type']    = 'dv2_album'; //'page'; //modified by zMinion
	        $page['post_content'] = apply_filters('ngg_add_page_shortcode', '[nggallery id="' . $this->gid . '"]' );
            $page['post_parent']  = $parent_id;
            $page['post_author']  = $user_ID;
            $page['post_status']  = 'draft'; //'publish'; //modified by zMinion
            ...
            if ($gallery_pageid != 0)
            {
				...
				update_post_meta($gallery_pageid, 'ecpt_galleryid', $this->gid); //added by zMinion
				
				$_POST['pageid'] = $gallery_pageid; //added by zMinion
				//this hook was added by zMinion
				do_action('ngg_update_gallery_only_featured_image', $this->gid, $_POST); //added by zMinion				
            }
